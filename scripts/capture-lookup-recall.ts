...
if (key === undefined || key.length === 0) {
  console.warn(
    "warning: no GOOGLE_BOOKS_API_KEY — Google will 429 and the corpus will be wrong",
  );
}
...
for (const entry of RECALL_CORPUS) {
  await lookup(
    entry.term,
    recording,
    key === undefined ? {} : { googleBooksKey: key },
  );
}
...
console.log(
  `${Object.keys(recorded).length} responses from ${RECALL_CORPUS.length} lookups`,
);
