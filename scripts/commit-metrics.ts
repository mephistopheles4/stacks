...
const found = records();
if (found.length === 0) {
  console.error(`no .prom file in ${SOURCE} — emit-metrics.ts writes one per run, so this is a bug`);
  process.exit(1);
}
...
try {
  ...
  for (let attempt = 1; attempt <= 3 && !pushed; attempt += 1) {
    pushed = gitStatus(['push', 'origin', `HEAD:refs/heads/${BRANCH}`], worktree) === 0;
    if (pushed) break;
