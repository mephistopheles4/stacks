...
const covers = readdirSync(SOURCE_COVERS).filter(
  (file) =>
    file.endsWith(".png") &&
    !file.startsWith("all-white") &&
    !file.startsWith("white-bordered"),
);
...
for (let i = 0; written < BOOK_COUNT; i += 1) {
  ...
  if (status === "read" || status === "abandoned")
    lines.push(`started: ${year}-${month}-${day}`);
  if (status === "read") lines.push(`finished: ${year}-${month}-${day}`);
  if (status === "read" && random() < 0.7)
    lines.push(`rating: ${1 + Math.floor(random() * 5)}`);
  if (cover !== undefined) {
  ...
  if (identified) {
    ...
    lines.push(
      `publisher: "${pick(["Meridian House", "Coldwater Press", "Underhill & Sons"])}"`,
    );
