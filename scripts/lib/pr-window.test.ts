...
describe("windowFrom — subjects to the window panel 1 shows", () => {
  it("reads the squash-merge suffix this repo actually writes", () => {
    expect(windowFrom(["feat: the scope list gets a gate (#179)"])).toBe(
      "#179",
    );
  });
  ...
  it("ignores a number that is merely mentioned, and takes only the suffix", () => {
    ...
    expect(windowFrom(["fix: strike inference from #52 permanently"])).toBe(
      NO_WINDOW,
    );
    expect(
      windowFrom(["fix: strike inference from #52 permanently (#153)"]),
    ).toBe("#153");
  });
  ...
  it("counts a pull request once, however many commits name it", () => {
    ...
    expect(
      windowFrom(["feat: a thing (#180)", "fix: follow-up to the same (#180)"]),
    ).toBe("#180");
  });
  ...
  it("takes a real merge commit too, which this repo does not write but git does", () => {
    ...
    expect(
      windowFrom(["Merge pull request #124 from mephistopheles4/feat/thing"]),
    ).toBe("#124");
  });
});
