...
describe("parsing a record", () => {
  ...
  it("reads a label set rather than splitting on the comma inside one", () => {
    ...
    expect(info?.["run_url"]).toBe(
      "https://github.com/mephistopheles4/stacks/actions/runs/1",
    );
  ...
  it("counts a family with no samples as emitted, dated by the record", () => {
    ...
    const document = nightly(NOW, {
      mutationScore: [{ scope: "packages/core/src", score: null }],
    });
...
describe("judging a record — per-series, because the record is not one number", () => {
  it("passes a nightly inside the bound", () => {
    expect(
      judgeRecord({ now: NOW, records: [parseRecord(nightly(NOW - 3600))] }),
    ).toEqual({
      kind: "fresh",
    });
  });
  ...
  it("refuses the series that went quiet, not the record", () => {
    ...
    expect(
      verdict.kind === "stale" ? verdict.stale.map((one) => one.series) : [],
    ).toEqual(["mutation-score", "mutation-run-runtime", "live-exclusions"]);
    expect(
      verdict.kind === "stale" ? verdict.stale[0]?.newest : undefined,
    ).toBe(NOW - 4 * DAY);
  });
  ...
  it("treats a series with no sample at all exactly as a stale one", () => {
    ...
    const verdict = judgeRecord({
      now: NOW,
      records: [parseRecord(merge(NOW))],
    });

    expect(verdict.kind).toBe("stale");
    expect(
      verdict.kind === "stale" ? verdict.stale.map((one) => one.newest) : [],
    ).toEqual([undefined, undefined, undefined]);
  });

  it("holds the bound at three days rather than near it", () => {
    const at = (age: number): string =>
      judgeRecord({ now: NOW, records: [parseRecord(nightly(NOW - age))] })
        .kind;
  ...
  it("refuses four days after the spine landed, with no record", () => {
    expect(judgeRecord({ now: SPINE + 4 * DAY, records: [] })).toEqual({
      kind: "never",
      days: 4,
    });
  });
  ...
  it("expires the exemption on the third day, not the fourth", () => {
    ...
    expect(
      judgeRecord({ now: SPINE + STALE_AFTER_DAYS * DAY, records: [] }).kind,
    ).toBe("never");
    expect(
      judgeRecord({ now: SPINE + STALE_AFTER_DAYS * DAY - 1, records: [] })
        .kind,
    ).toBe("bootstrap");
  });
  ...
  it("refuses a deploy when one complexity series goes quiet and the rest do not", () => {
    ...
    const quiet = judgeRecord({
      now: NOW,
      records: [nightly(NOW - 600, { complexity: undefined })].map((text) =>
        parseRecord(text),
      ),
    });
    ...
    expect(
      quiet.kind === "stale" ? quiet.stale.map((one) => one.series) : [],
    ).toEqual([
      "complexity-functions",
      "complexity-mass",
      "complexity-mass-over-10",
...
describe("reading one trend off a record", () => {
  it("reads a per-scope sample by trend name, not by metric string", () => {
    ...
    expect(samplesOf(record, "complexity-mass").get("packages/core/src")).toBe(
      340,
    );
    expect(
      samplesOf(record, "complexity-mass-over-10").get("packages/cli/src"),
    ).toBe(22);
    expect(samplesOf(record, "complexity-max").get("packages/core/src")).toBe(
      21,
    );
  });
  ...
  it("answers alike for the same question asked twice", () => {
    ...
    expect([...scoresOf(record)]).toEqual([
      ...samplesOf(record, "mutation-score"),
    ]);
  });
});
...
describe("which half of metrics.yml wrote a record", () => {
  it("reads push as the merge half and everything else as the nightly", () => {
    ...
    expect(
      halfOf(parseRecord(nightly(NOW, { event: "workflow_dispatch" }))),
    ).toBe("nightly");
  });
...
describe("the records a delta compares", () => {
  ...
  const store = (...documents: string[]) =>
    documents.map((text) => parseRecord(text));
  ...
  it("skips a record written before the series existed", () => {
    ...
    const records = store(
      merge(NOW),
      merge(NOW - DAY, { complexity: undefined }),
    );
  ...
  it("refuses to pair two records whose half is unknown", () => {
    ...
    expect(deltaPair(records, "complexity-mass")).toEqual({
      latest: records[0],
    });
  });
  ...
  it("still pairs when the half is known on both", () => {
    ...
    const { previous } = deltaPair(
      store(merge(NOW), merge(NOW - DAY)),
      "complexity-mass",
    );
  ...
  it("offers nothing at all when no record carries the series", () => {
    ...
    expect(
      deltaPair(
        store(merge(NOW, { complexity: undefined })),
        "complexity-mass",
      ),
    ).toEqual({});
