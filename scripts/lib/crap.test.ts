...
function fn(
  over: Partial<PerFunction> & { line: number; complexity: number },
): PerFunction {
  return {
...
describe("coverageByFunction", () => {
  ...
  it("reads a null end column as the end of the line, not as column zero", () => {
    ...
    const openEnded: IstanbulFile = {
      statementMap: {
        "0": { start: { line: 9, column: 4 }, end: { line: 9, column: 20 } },
      },
      fnMap: {
        "0": {
          name: "ends",
    ...
    expect(coverageByFunction(openEnded).get("0")).toEqual({
      hit: 1,
      total: 1,
      fraction: 1,
    });
...
describe("fileCoverageOf", () => {
  ...
  it("finds a repo-relative file among absolute report keys", () => {
    ...
    expect(
      fileCoverageOf(report, "packages/core/src/identity.ts", "/repo"),
    ).toBe(planted);
  });
  ...
  it("is absent — never zero — for a file the report does not carry", () => {
    ...
    const report = { "/repo/packages/core/src/identity.ts": planted };
    expect(
      fileCoverageOf(report, "packages/core/src/frontmatter.ts", "/repo"),
    ).toBeUndefined();
  });
});
...
describe("rowsFor", () => {
  ...
  it("joins ESLint’s report position to Istanbul’s declaration line", () => {
    const row = only(
      rowsFor([fn({ line: 11, complexity: 12, name: "uncovered" })], file),
    );
...
describe("route", () => {
  const declarations: Declarations = {
    ...
    scopes: [
      ...
      },
      {
        ...
        exclusions: [
          {
            path: "packages/site/src/shelf/scene.ts",
            mechanism: "the browser drives it",
          },
        ],
      },
    ],
  ...
  it("measures a file inside a declared scope", () => {
    expect(
      route(["packages/core/src/identity.ts"], declarations).measured,
    ).toEqual(["packages/core/src/identity.ts"]);
  });
  ...
  it("holds an excluded file out with its mechanism", () => {
    ...
    expect(routed.excluded).toEqual([
      {
        file: "packages/site/src/shelf/scene.ts",
        mechanism: "the browser drives it",
      },
    ]);
  });
  ...
  it("leaves a file no scope declares outside, silently", () => {
    const routed = route(
      ["docs/plan.md", "packages/cli/src/index.ts"],
      declarations,
    );

    expect(routed.measured).toEqual([]);
    expect(routed.outside).toEqual([
      "docs/plan.md",
...
describe("renderReport", () => {
  ...
  it("carries the never-calibrated caveat on the line that names CRAP", () => {
    ...
    const header = renderReport(rows, {
      measured: [],
      excluded: [],
      outside: [],
    })
