...
describe("the panel", () => {
  ...
  it("distinguishes an empty window from an unmeasurable one", () => {
    ...
    expect(
      panel([scored(NOW, 0.71), scored(NOW - DAY, 0.71, "c")], { window: [] }),
    ).toContain("no pull request merged");
    expect(
      panel([scored(NOW, 0.71), scored(NOW - DAY, 0.71, "c")], {
        window: undefined,
      }),
  ...
  it("carries the per-mutant resolution beside the score", () => {
    ...
    expect(text).toContain(
      "killed 512, timeout 3, survived 190, no coverage 12",
    );
  ...
  it("says so when a record does not report its health at all", () => {
    ...
    expect(panel([unstated])).toContain(
      "does not say whether its run completed",
    );
  });
...
const complexity = (records: readonly ParsedRecord[]): string[] =>
  renderComplexity(records, NOW);
...
describe("the complexity block", () => {
  it("spells the same four counts the record does, in the same order", () => {
    ...
    expect(COMPLEXITY_COUNTS.map(([series]) => series)).toEqual(
      COMPLEXITY_SERIES,
    );
  });
  ...
  it("prints four lines per scope, one for each count", () => {
    ...
    expect(
      lines.filter((line) => line.includes("packages/core/src")),
    ).toHaveLength(4);
    expect(
      lines.filter((line) => line.includes("packages/cli/src")),
    ).toHaveLength(4);
  });
  ...
  it("reads a merge against the previous merge, stepping over the nightly between", () => {
    ...
    const text = complexity([
      ...
      counted(
        NOW - DAY,
        [core({ functions: 999, mass: 9999, massOver10: 999, max: 99 })],
        { event: "schedule" },
      ),
...
describe("the refusal", () => {
  ...
  it("keeps a name off its own explanation, however long the name is", () => {
    ...
    const long = {
      kind: "stale" as const,
      stale: [
        { series: "complexity-mass-over-10" },
        { series: "mutation-score" },
      ],
    };
  ...
  it("sends you to Actions when the branch is no fresher", () => {
    ...
    const text = renderRefusal(
      stale,
      NOW,
      { kind: "same", branchNewest: "2026-08-14" },
      5,
    );
