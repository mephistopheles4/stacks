...
function clauses(declarations: Declarations, files: readonly string[]): Clause[] {
...
describe('declarationFaults — every clause, planted', () => {
  ...
  it('catches a glob that matches nothing while its directory still exists', () => {
    ...
    declarations.scopes[0] = scope('packages/core/src', 'packages/core/src/moved/**/*.ts');
  ...
  it('catches an exclusion naming a file that no longer exists', () => {
    const declarations = healthy();
    declarations.scopes[2] = scope('scripts', 'scripts/**/*.ts', [
      { path: 'scripts/deploy-old.ts', mechanism: 'a mechanism attached to nothing' },
    ]);
  ...
  it('catches two scopes claiming the same file', () => {
    ...
    expect(clauses(declarations, TREE).filter((clause) => clause === 'overlap')).toHaveLength(2);
  });

  it('refuses a directory that is declared and excluded at once', () => {
    const declarations = healthy();
    declarations.excludedDirectories.push({ path: 'scripts', mechanism: 'both, somehow' });
...
describe('emptyScopes — the clause the disk cannot answer', () => {
  const scopes = [scope('scripts', 'scripts/**/*.ts'), scope('gates', 'gates/**/*.ts')];
  ...
  it('names a scope whose files exist and whose mutants do not', () => {
    ...
    expect(emptyScopes(report({ 'scripts/lib/walk.ts': ['Killed'] }), scopes, files)).toEqual([
      'gates',
    ]);
  });
  ...
  it('reads the tally and not the score', () => {
    ...
    const survived = report({ 'scripts/lib/walk.ts': ['Survived'], 'gates/repo.ts': ['Killed'] });
  ...
  it('stays silent about a scope whose glob matches nothing on disk', () => {
    ...
    expect(emptyScopes(report({}), scopes, ['scripts/lib/walk.ts'])).toEqual(['scripts']);
  });
});
