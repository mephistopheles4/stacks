...
describe("stampOf", () => {
  ...
  it("reads back exactly what the deploy writes", () => {
    ...
    // and says so nowhere.
    expect(stampOf(`<head>${stampMeta("0123456789ab")}</head>`)).toBe(
      "0123456789ab",
    );
  });
});
...
describe("describeStaleCover", () => {
  it("names the cover, what is served, and what was built", () => {
    ...
    expect(
      describeStaleCover({ cover: "covers/x.png", served: 0, built: 619 }),
    ).toBe("covers/x.png: serving 0B, built 619B");
  });
});
...
describe("probeBuild", () => {
  ...
  it("waits out the edge before calling a different build stale", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      answering([ok(page("9f9f9f9f9f9f")), ok(page(STAMP))]),
    );
  ...
  it("tells the caller about each retry, so a long wait is not a silent one", async () => {
    ...
    await probeBuild(ORIGIN, STAMP, {
      ...QUICK,
      onRetry: (message) => said.push(message),
    });
...
describe("probeCovers", () => {
  function head(
    sizes: Record<string, number | "refused" | "gone">,
  ): typeof fetch {
  ...
  it("names every cover served at a size the build did not produce", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      head({ "covers/one.jpg": 100, "covers/two.jpg": 99 }),
    );
  ...
  it("counts an answer with no content-length as uncomparable, never as 0 bytes", async () => {
    ...
    expect(
      await probeCovers(ORIGIN, new Map([["covers/one.jpg", 619]])),
    ).toEqual({
      kind: "checked",
      checked: 1,
      stale: [],
  ...
  it("reports a refusal rather than reading a challenge page as a byte mismatch", async () => {
    ...
    expect(
      await probeCovers(ORIGIN, new Map([["covers/one.jpg", 100]])),
    ).toEqual({
      kind: "refused",
      status: 403,
    });
