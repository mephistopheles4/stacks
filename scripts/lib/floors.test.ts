...
describe("correspondence", () => {
  ...
  it("reports a declared scope that no entry accounts for", () => {
    ...
    expect(
      correspondence(declared, parseFloors(WELL_FORMED)).unaccounted,
...
const SCOPES = [
  ...
  scope("scripts", "scripts/**/*.ts", [
    { path: "scripts/deploy.ts", mechanism: "run by tsx" },
  ]),
];
      ...
      [
        {
          path: "packages/core/src/library.ts",
          source:
            'const help = "Stryker disable next-line withholds a mutant";\n',
        },
      ],
      SCOPES,
    );
...
describe("configHashOf", () => {
  ...
  it("moves when the timeout moves", () => {
    expect(configHashOf({ ...CONFIG, timeoutMS: 15000 })).not.toBe(
      configHashOf(CONFIG),
    );
  });
  ...
  it("moves when the mutated population moves", () => {
    expect(
      configHashOf({ ...CONFIG, mutate: ["packages/core/src/*.ts"] }),
  ...
  it("stands still for a field that only decides where output is written", () => {
    expect(configHashOf({ ...CONFIG, reporters: ["clear-text"] })).toBe(
      configHashOf(CONFIG),
    );
    expect(
      configHashOf({ ...CONFIG, jsonReporter: { fileName: "elsewhere.json" } }),
    ).toBe(configHashOf(CONFIG));
  });
  ...
  it("moves for an option nobody has classified", () => {
    expect(configHashOf({ ...CONFIG, ignoreStatic: true })).not.toBe(
      configHashOf(CONFIG),
    );
  });
});
...
describe("breaches", () => {
  ...
  it("does not refuse a score sitting exactly on its floor", () => {
    expect(
      breaches(
        [{ scope: "packages/core/src", score: 71.55, mutants: 1250 }],
        FLOORS,
      ),
    ).toEqual([]);
  });
  ...
  it("never refuses an unarmed scope, whatever it scored", () => {
    expect(
      breaches([{ scope: "scripts", score: 3.1, mutants: 400 }], FLOORS),
    ).toEqual([]);
  });
  ...
  it("refuses nothing on a scope the record carries no score for", () => {
    expect(
      breaches([{ scope: "packages/core/src", score: null }], FLOORS),
    ).toEqual([]);
  });
...
describe("calibration", () => {
  ...
  it("is not full one run short", () => {
    expect(calibration(nightlies(19), ["packages/core/src"], HASH).full).toBe(
      false,
    );
  });
  ...
  it("counts the nightlies it considered, whether or not any qualified", () => {
    const rows = nightlies(11).map((row) => ({
      ...row,
  ...
  it("takes each scope its lowest score across the window", () => {
    ...
    expect(
      calibration(rows, ["packages/core/src"], HASH).lowest.get(
        "packages/core/src",
      ),
    ).toBe(66.12);
  });
...
describe("floorRefusals", () => {
  ...
  it("says on every refusal that no flag clears it", () => {
    const all = [
      ...floorRefusals({
        ...CLEAN,
        readings: [{ scope: "packages/core/src", score: 1 }],
      }),
      ...floorRefusals({
        ...CLEAN,
        declared: [...CLEAN.declared, "packages/new"],
  ...
  it("does not refuse an unstamped row while every scope is unarmed", () => {
    ...
    expect(
      floorRefusals({ ...CLEAN, floors: unarmed, run: {}, readings: [] }),
    ).toEqual([]);
  });
...
describe("runRowsFrom", () => {
  ...
  it("reads a run as its health, its configuration and its per-scope scores", () => {
    const [row] = runRowsFrom([
      parseRecord(ci(1_760_000_000, 1, "sha256:abc", 0.7155)),
    ]);
  ...
  it("reads a crashed run as unhealthy rather than dropping it", () => {
    const [row] = runRowsFrom([
      parseRecord(ci(1_760_000_000, 0, "sha256:abc", 0.12)),
    ]);
  ...
  it("carries the event, which is what tells a nightly from a merge", () => {
    const [row] = runRowsFrom([
      parseRecord(ci(1_760_000_000, 1, "sha256:abc", undefined, "push")),
    ]);
...
describe("nightliesIn", () => {
  ...
  it("keeps a dispatched run, which is the nightly half run by hand", () => {
    const rows = [
      row(1_760_000_000, "workflow_dispatch", new Map([["scripts", 50]])),
    ];
...
describe("countDisableDirectives — the spellings a comment can take", () => {
  ...
  const forms: Record<string, string> = {
    ...
    "continuation line": "/*\n * Stryker disable next-line all\n */\n",
    "a named mutator rather than all":
      "// Stryker disable next-line ArithmeticOperator\n",
    "a trailing reason":
      "// Stryker disable next-line all: the survivor is unreachable\n",
  ...
  it("still refuses to count the words where no comment opens", () => {
    const counted = countDisableDirectives(
      [
        {
          path: "packages/core/src/library.ts",
          source: 'const s = "Stryker disable all";\n',
        },
      ],
      SCOPES,
    );
...
describe("fixtureHashOf", () => {
  ...
  it("changes when either installed version changes", () => {
    expect(fixtureHashOf(inputs({ eslintVersion: "10.9.1" }))).not.toBe(
      fixtureHashOf(inputs()),
    );
    expect(fixtureHashOf(inputs({ parserVersion: "8.68.0" }))).not.toBe(
      fixtureHashOf(inputs()),
    );
  });
  ...
  it("hashes the two versions positionally, not as a set", () => {
    const swapped = inputs({
      eslintVersion: "8.67.0",
      parserVersion: "10.9.0",
    });
  ...
  it("changes when the rule is configured differently", () => {
    expect(
      fixtureHashOf(inputs({ ruleOptions: [{ max: 5, variant: "classic" }] })),
    ).not.toBe(fixtureHashOf(inputs()));
    // ESLint's defaults are `max: 20`, so no options at all is a different rule
    // and reads as one rather than as an absence.
    expect(fixtureHashOf(inputs({ ruleOptions: [] }))).not.toBe(
      fixtureHashOf(inputs()),
    );
  });
...
describe("parseFloors, the cap half", () => {
  ...
  it("requires a fixtureHash the way it requires a configHash", () => {
    ...
    expect(() => parseFloors({ ...WELL_FORMED, fixtureHash: "" })).toThrow(
      /fixtureHash/,
    );
  });
  ...
  it("keeps `unarmed` as a cap value distinct from every number", () => {
    ...
    expect(caps?.get("complexity-mass-over-10")?.notes).toEqual([
      "raised once",
    ]);
  });
  ...
  it("treats a missing caps object as no caps rather than as a fault", () => {
    expect(
      parseFloors(document(undefined)).scopes.get("scripts")?.caps.size,
    ).toBe(0);
  });
  ...
  it("throws on a cap naming a series that cannot be capped", () => {
    expect(() =>
      parseFloors(document({ "complexity-maxx": BOTH["complexity-max"] })),
    ).toThrow(/complexity-maxx/);
    expect(() =>
      parseFloors(document({ "complexity-mass": BOTH["complexity-max"] })),
    ).toThrow(/complexity-mass/);
  });
  ...
  it("throws on a cap that is neither a number nor unarmed", () => {
    expect(() =>
      parseFloors(
        document({
          "complexity-max": { cap: "later", armed: "2026-08-22", notes: [] },
        }),
      ),
  ...
  it("throws on a cap carrying no date, so `unarmed` cannot be typed in blind", () => {
    expect(() =>
      parseFloors(
        document({ "complexity-max": { cap: "unarmed", notes: [] } }),
      ),
    ).toThrow(/date/);
  });
  ...
  it("throws on notes that are not a list of lines", () => {
    expect(() =>
      parseFloors(
        document({
          "complexity-max": { cap: 4, armed: "2026-08-22", notes: "why" },
        }),
      ),
    ).toThrow(/notes/);
  });
...
describe("CAPPED_SERIES", () => {
  ...
  it("names the cut in the series that measures it", () => {
    expect(CAPPED_SERIES).toContain(
      `complexity-mass-over-${String(MCCABE_CUT)}`,
    );
  });
});
...
describe("runRowsFrom, the cap half", () => {
  ...
  it("reads the counting stamp beside the scoring one", () => {
    const [row] = runRowsFrom([
      parseRecord(counted(1_760_000_000, "sha256:counted", 12)),
    ]);
  ...
  it("leaves a row written before the counting stamp existed with no fixture hash", () => {
    ...
    expect(
      runRowsFrom([parseRecord(document)])[0]?.fixtureHash,
    ).toBeUndefined();
  });
  ...
  it("reads a record that carried no counts as carrying none", () => {
    const [row] = runRowsFrom([
      parseRecord(counted(1_760_000_000, "sha256:counted")),
    ]);
...
describe("capBreaches", () => {
  ...
  it("names the scope, the series, the value and the cap", () => {
    ...
    expect(found).toEqual([
      { scope: "scripts", series: "complexity-max", value: 13, cap: 12 },
    ]);
  });
  ...
  it("does not breach on a value sitting exactly on its cap", () => {
    expect(
      capBreaches(
        [{ scope: "scripts", series: "complexity-max", value: 12 }],
        FLOORS,
      ),
    ).toEqual([]);
  });
  ...
  it("refuses nothing for an unarmed cap, however large the value", () => {
    expect(
      capBreaches(
        [{ scope: "scripts", series: "complexity-mass-over-10", value: 999 }],
        FLOORS,
      ),
    ).toEqual([]);
  });
  ...
  it("refuses nothing for a scope that has no cap entry at all", () => {
    expect(
      capBreaches(
        [{ scope: "packages/cli/src", series: "complexity-max", value: 999 }],
        FLOORS,
      ),
    ).toEqual([]);
  });
  ...
  it("invents no verdict where the record carried no value", () => {
    expect(
      capBreaches(
        [{ scope: "scripts", series: "complexity-max", value: null }],
        FLOORS,
      ),
    ).toEqual([]);
  });
...
describe("capCalibration", () => {
  ...
  function runs(count: number, max = 10): RunRow[] {
    return Array.from({ length: count }, (_, index) =>
      row(1_760_000_000 + index * 86_400, max),
    );
  }
  ...
  it("arms at the highest value observed, which is the cap rule", () => {
    ...
    expect(
      capCalibration(rows, ["scripts"], HASH)
        .highest.get("complexity-max")
        ?.get("scripts"),
    ).toBe(17);
  });
  ...
  it("breaks the streak on a failed run and on a gap over three days", () => {
    ...
    const gapped = runs(20).map((entry, index) =>
      index < 10
        ? { ...entry, timestamp: entry.timestamp - 10 * 86_400 }
        : entry,
    );
...
describe("renderCapLines", () => {
  ...
  it("states an armed cap, the current count and the move since the last record", () => {
    const lines = renderCapLines({
      floors: FLOORS,
      readings: [
        { scope: "scripts", series: "complexity-max", value: 11, previous: 13 },
      ],
      window: window(4),
      today: "2026-08-25",
    });
  ...
  it("says so when an armed cap has no count in the record", () => {
    ...
    expect(lineFor("complexity-max", lines)).toContain(
      "no count in the record",
    );
  });
  ...
  it("reports a hole rather than offering a cap it cannot derive", () => {
    ...
    expect(lineFor("complexity-mass-over-10", lines)).toContain(
      "no complete history",
    );
  });
