...
A miniature Obsidian vault used by every phase's tests and gates. It mirrors the
real layout: notes in `Library/`, covers cached in `Library/covers/`, so a note's
`cover:` value stays relative to the note itself.
