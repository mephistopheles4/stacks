...
**Core architectural bet:** no separate database. Frontmatter in book notes *is* the data model. This makes the tool Obsidian-native, git-friendly, and trivially portable. If this bet fails (frontmatter too limiting), fall back to a `library.json` index generated from the vault.
...
- **Blocking (decide in Phase 0):** Astro island with react-three-fiber vs. vanilla Three.js in a plain Vite page? R3F is nicer for interaction; vanilla is lighter. Recommend R3F, but decide before scaffolding. *(Owner: you)*
