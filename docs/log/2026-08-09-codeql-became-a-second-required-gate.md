...
that enabled CodeQL, that requiring it was *"worth doing once its findings here
have been triaged rather than before"*, and this is that condition being met
...
Capping the input to 120 characters *before* the strip makes it 0.2ms.
...
two-second threshold *while testing the very defect it was written for*. Measured
...
procedure is written down in [`gates.md`](../gates.md) under *"Triaging a CodeQL
finding"* so the next batch does not re-litigate it.

**It is a ruleset rule, not a required status check, and
[ADR-0023](../adr/0023-ci-shape.md) is why** — *"a required check that never
reports blocks the pull request forever."* `CodeQL` is posted by the
...
listed in [`gates.md`](../gates.md) under *"Not gated, deliberately"*.
...
ADRs with *different filenames* and the same number collide in nothing git looks
...
identical race for scoreboard rows, *"what number a row will carry is not
knowable until it lands"*. Unlike gate rows, nothing asserts ADR numbers are
unique. Not fixed.
