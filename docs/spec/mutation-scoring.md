...
explicit revisit condition — _"Revisit once the rows above are green"_ — and all
...
_"133 tests in ~2s"_; measured at `1d0548f`, the suite is **636 tests across 66
files in 5.52s**. Mutation cost scales with suite runtime, so the premise that
made Stryker _"genuinely cheap"_ is weaker than recorded. The row is corrected —
...
(2026-04-10) is the _first_ release that works against Vitest 4; its changelog
entry is _"vitest-runner: fix vitest runner mutant hitcount and coverage for
v4.1"_, closing an issue titled _"No proper coverage with Vitest 4 … completely
uncovered functions are mutated and affect the resulting score."_ **9.6.0
...
_evidence_ is the changelog and the code, never the range.
...
cleanest specimen this effort found of a range _asserting_ a compatibility that
...
_"Dead and cannot be revived"_ was a fact about `typescript@7.0.2`, and the repo
...
reason — a `CompileError` is neither killed nor survived, so turning it on moves
every declared scope's number and restarts every calibration window behind
`stryker.floors.json`. Off by decision now, not by impossibility. The paragraph
above is kept because it is what the configuration was written against, and what
returns if the pin ever moves back.
...
each time: reading a published tarball sees what the source _says_, not what the
run _does_. **An implementation session that skips these gets three failures
whose messages point somewhere else.**

1. **`@stryker-mutator/core` _does_ touch `typescript`, and Stryker crashes on
...
counts as _detected_, and the score reads **higher**. A floor calibrated on a
...
| **source-text regex**                  | G14 reads `packages/cli/src/index.ts` as _text_                      | nothing — no import edge, so `vitest.related` never pulls the spec in |
...
_not_ untested, since **G14 checks it by regex over its source text**, which kills
...
**This is the transferable half of the whole piece.** _Score only what an
in-process oracle reaches, and name the mechanism that puts everything else out of
reach_ transfers to any codebase; the eight scopes below do not.

---

## 4. Eight declared scopes — reachable only, declared never discovered

**Directory rollups first** — these are _packages_, not declared scopes, and the
...
scope** at 1227 mutants / 71.7% _and_ a **directory rollup** at 3,301 / 66.6%.
...
⚠️ **#114's _"all 18 `packages/core/src/*` directories"_ is 5** — the glob matches
...
`scripts/lib/public-build.ts` is 386 _lines_, which is not a mutant count. **The
...
put and rejected: it derives the scope _structure_ from the measurement, which is
...
`NoCoverage` and both score 0. Excluding _"anything at 100% NoCoverage"_ would be
...
gap, and it stays in the denominator.** _"It is only glue"_ is the most available
...
twenty checks if it is wrong. Both are unreachable _because of how they are
structured_. **The exclusion list therefore doubles as a register of them**, which
...
⚠️ **And _"no spec imports them"_ is a mechanism the attacker controls** — unlike
_"the child never sees the active mutant"_, which is a property of the harness.
...
the tree**. _A gate that cannot tell it is being run against the wrong tree is not
yet a gate either._

**The return condition is written down**, which makes _out_ a reversible
...
_"A gate is the last thing in the chain, so nothing can notice it being wrong"_ is
...
it, and it has no spec of its own.** Those 20 gates _are_ its oracles, so it is the
...
**A scope list living only in `stryker.config` is _a rule nothing can fail on_.**
...
assertion _and_ a deploy refusal under one slug, and `docs/gates.md` has no column
...
the owner's question was _"if we block then we should fix something, what would we
fix?"_, which is Clause A applied to a refusal that had never been tested against
...
_Delete the scope_ is a legitimate fix **and** the cheapest way to stop measuring
...
the residual check is _written_, never whether it is wanted. Expect the worst of
...
**Static mutants cannot live in the scope list at all.** `mutate` selects _files_;
`static` is a property of individual _mutants within_ a file. So it is a score-time
...
| [`docs/gates.md`](../gates.md)                            | **row G37 `mutation-scope`, _Contract seams → gates_**                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| [`docs/gates.md`](../gates.md), _Not gated, deliberately_ | the **Mutation testing (Stryker)** row **marked in place, dated, with the corrected count beside the wrong one** — see below                                                                                                                                                                                                                                                                                                                                                                                           |
...
mechanism is for _numbered_ rows — _"keeps its number and its row"_ — and this
...
```markdown
**Mutation testing (Stryker)** — _Genuinely cheap here … Revisit once the rows
above are green._ ⚠️ **Revisited 2026-08-11: condition met, and the cost estimate
...
⚠️ **The ratio between those two numbers is not `4.8×`.** That is the _test-count_
...
`docs/gates.md`'s standard is _"a gate never observed failing is not yet a gate"_,
and a statistical check makes that harder rather than optional.

| Check                   | Plant this                                                   | Expect                                                 |
| ----------------------- | ------------------------------------------------------------ | ------------------------------------------------------ |
| **G37**, structural     | rename a scope's directory without editing the config        | red: _declared scope does not exist on disk_           |
...
| **the score itself**    | delete a test that killed two mutants in `packages/core/src` | the nightly moves ~0.16 points — _reported, never red_ |
