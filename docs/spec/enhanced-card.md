...
decision 4 — _what you brought to the book, then what the book is, then the exit_
...
"about the book" block, which **never vanishes**. The _object line_ drops when
...
`describe()` those render nothing at all. Once the line is _about_ reading, an
...
stated once so it cannot be read two ways: _the drag is inert above the
breakpoint, and the breakpoint is expressed once and read by both._ The mechanism
...
_content_ differ by breakpoint and why [#106](https://github.com/mephistopheles4/stacks/issues/106)
...
reintroduces ambiguity _intermittently_ — with a content-capped sheet most books
...
the _only_ way they learn what they hit, which is why it is short rather than
complete.
...
| Swap            | **Re-announces**, by the same mechanism — the status text changes. Tapping the _same_ book twice announces nothing.                                                                                                     |
...
card cannot announce the card _opening_ at all.
...
**Keyboard reachability of the shelf is out of scope** and is on the map's _Out
of scope_ with a revisit condition: the "focus does not move on open" decision
...
search link instead.** The distinction rides in the _form_ rather than in a
...
| search fallback | the text "Search Open Library" | — (visible text _is_ the name) |

- **Dropping `aria-label` is what makes `title` legal again.** The double-announce
  this repo already documents (`index.astro:64-68`) needs _both_ attributes
...
- **Google's name is `Google Preview`, not "Google Books".** Google's grant is a
  _button whose artwork is an image of those words_; naming it for the
...
- **Styles stay in `Shelf.astro`'s `<style>` block**, extending the existing
  ...
  elements _in the template_; nothing built with `createElement` carries it, so
