...
| Complexity on the trend layer — four counts, no ratio, and CRAP decided on its merits | [`complexity-on-the-trend-layer.md`](complexity-on-the-trend-layer.md) | **waiting for an implementation session**. Adds four series to the trend layer and **no gate row**; every decision in it was made overnight by Claude on the owner's instruction, and [the map](https://github.com/mephistopheles4/stacks/issues/186) carries the override list |
