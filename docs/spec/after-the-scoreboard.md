...
| **CI writes an immutable record; the machine pulls it** | The record and the store are separable, so no hosted option's ingest window can cost history. Buys a no-secret design and unlimited replay; costs an unprotected branch holding the calibration evidence for every armed floor. | [#121](https://github.com/mephistopheles4/stacks/issues/121) |
