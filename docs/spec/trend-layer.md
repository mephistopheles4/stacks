...
**It landed, and the predicted shape held**: a `csp` entry in `PUBLIC_BUILD_RULES`
inherited G20's observed-red obligation for free, with no new row. Two corrections to
the prediction, both in [ADR-0065](../adr/0065-the-csp-is-generated-not-written.md) —
the policy is generated by Astro as a per-page `<meta http-equiv>` rather than
hand-written in `_headers`, and `script-src` names
`https://static.cloudflareinsights.com`, because the zone injects a Web Analytics
beacon this repo does not contain. Refusing it is a real choice rather than a no-op —
the analytics would stop — but the beacon reports same-origin and carries nothing
derived from the owner's reading, so a policy file overriding a zone setting buys
none of what this principle protects.
