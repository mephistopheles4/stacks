...
never built.** G50 (`astro-types`) installed `@astrojs/check@0.9.10` on
...
and the `### G50 — \`astro-types\`` entry, whose own **Decay** verdict is this
...
**Live-run evidence:** owed. The `edited` trigger is a claim about GitHub and no local test runs one; the two run URLs — red on a title edited to non-conforming, green on it edited back — go here.
**Triaged at landing**, per G41.
