...
| ------------------------------------------- | ----------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `renderer.toneMapping`                      | Renderer property, one of seven constants | **Live, and three recompiles for you.** It is in the program cache key (`WebGLPrograms.js:484`) _and_ in the `needsProgramChange` chain (`WebGLRenderer.js:2482`), so no `material.needsUpdate` is needed — measured below | +1 uniform, +100 shader lines per program. Free per fragment. **But every distinct value permanently adds one program per material class** — see "programs accumulate" | **Yes.** The largest single lever here                                                                                                                                                                                                                                                            |
...
| `MeshPhysicalMaterial` (clearcoat, sheen)   | Replace the material object               | Recompile, and a _different material class_ — not a toggle                                                                                                                                                                 | Standard 22 uniforms → physical 29–31. More varyings, bigger link                                                                                                      | **No.** On the device whose failure was a program that **would not link**, this is the wrong direction. Defer                                                                                                                                                                                     |
...
## What each tone mapping does to _this_ scene
...
| _authored `spine_color`_      | —        | —      | —          | —         | _0.411_   | _98.6_     | _0_                          |
...
- **The shipped render already desaturates a spine by about a fifth** — median
  saturation 0.323 against the authored 0.411 — while getting its _brightness_
...
- **On saturation alone, two operators fix what the shipped render breaks.**
  ...
  spine under ACES or Cineon reads _more_ like a real binding and measures
...
For completeness, the 41 books that _do_ carry a printed title, sampled the same
...
is `|pixel − background| > 12` per channel, in _either_ direction, so the
...
a second run — and then rises again as the scene darkens _past_ the background
...
What a _default_ change would do is break comparability:
...
_after_ `tonemapping_fragment` and `colorspace_fragment`
...
stays valid whatever is done here. Scope the claim, though — at _partial_ fog
...
2. Fog is the cheapest thing on this list to _remove_: 3 uniforms and one line
...
programs it builds carry 29–31 active uniforms against the standard material's 22. The repo's own `describeLinkFailure` prints `MAX_VARYING_VECTORS` and
`MAX_FRAGMENT_UNIFORM_VECTORS` precisely because a program that compiled would
not _link_ on the device this project cares about. A glossy dust jacket is not
...
session reported _no_ change at all from the same operation, for reasons never
...
the life of the page. Flipping _back_ is free — the cached program is reused — but the first
...
- **Recompile-on-flip is the phone risk**, not fragment cost. Nothing measurable
  here says a tone mapping curve is expensive to _run_; what is measurable is
