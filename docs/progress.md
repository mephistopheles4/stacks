...
| | |
| --- | --- |
| **Last green gate** | G52 (`sheet-size`) — no file under the site's public wood directory exceeds its long-edge or byte cap. It lands with the bookcase's veneer ([#302](https://github.com/mephistopheles4/stacks/issues/302), [the log](./log/2026-08-30-the-woodwork-is-rosewood.md)). ⚠️ **A whole asset class was outside every counter**: G15 (`cover-budget`) stages the fixture vault through `publish()` and measures what lands in `covers/`, and a file committed straight into `packages/site/public/` passes through none of that — so from the day the furniture stopped being flat colour, the largest images the shelf uploads were the ones nothing measured. ⚠️ **The cost is decode, not download** — `edge² × 4` bytes of RGBA, 4.0 MB at 1024 and **16.0 MB at 2048**, which is the arithmetic G15's own row records reaching the live site and killing a phone. ⚠️ **Neither cap is derived from what is in the directory today**, which would be a floor equal to a population: both are set against the sheets [#304](https://github.com/mephistopheles4/stacks/issues/304) and [#306](https://github.com/mephistopheles4/stacks/issues/306) will add, measured on their prototype branches. ⚠️ **A directory sweep passes perfectly when the directory is empty or renamed**, so the sweep is floored, a file `sharp` cannot open is a red naming it, and every URL `woodwork.ts` resolves must name a file the sweep found — ⚠️ **plural since [#304](https://github.com/mephistopheles4/stacks/issues/304)**, because a default page fetches two sheets once the backboard has its own, and a clause naming one leaves the other pointed at by nothing |
| **Before it** | G51 (`coplanar-faces`) — the bookcase has no coplanar, overlapping face pair, enumerated from the case's own dimension constants and counted at five row counts. **46 pairs on a four-row case, and they were on `main` from the day the case was built** ([#296](https://github.com/mephistopheles4/stacks/issues/296), [#301](https://github.com/mephistopheles4/stacks/issues/301), [the log](./log/2026-08-30-the-bookcase-stops-flickering.md)). ⚠️ **Only the backboard's flickered**, because it is a second material in a second colour while every other woodwork face carries the identical flat one — so a texture does not cause this, **it reveals it**, which is why the fix lands ahead of the woodwork treatment rather than inside it. ⚠️ **The backboard takes twice the plank's inset and that is not tidiness**: shrunk equally, its sides and the plank ends land on one *new* shared plane at `±1.786`, measured at 10 pairs and asserted by its own clause. ⚠️ **The count is derived as `8 × (rows + 1) + 6`, and the un-inset count is asserted first as the positive control** — an enumerator that has stopped matching reports zero on a geometry riddled with ties, and zero is the pass. ⚠️ **#296's own pair table sums to 48 against the 46 it states**; the arithmetic is right and the table is not |
