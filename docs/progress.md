...
| **A Cloudflare `HEAD` for a path this build does not have answers 200 with no `content-length`** | ⚠️ measured 2026-08-19 against the live origin: not a 404, and no size header — so the cover comparison reads `served === 0` and reports the cover as served at another build's size. **Pre-existing**, in `deploy:site` since the check was written and unchanged by the move into `lib/edge-probe.ts`. *Unmeasurable* and *stale* are different answers and this cannot yet tell them apart; see [the log](./log/2026-08-19-the-reading-half-lands-and-a-count-that-said-three.md) |
