...
| | |
| --- | --- |
| **Last green gate** | G44 (`stryker-reporters`) — the mutation run declares its Vitest reporters, because a list that resolved **empty** is what lets Vitest append its `github-actions` reporter, whose job summary appends once per `onTestRunEnd` — **once per mutant** under Stryker. 1054k against GitHub's 1024k on the nightly of 2026-08-22, and invisible on a laptop, where `GITHUB_ACTIONS` is unset. ⚠️ It landed beside a second, unrelated fault the same night — **G17 had been standing on the dated bootstrap** and expired with it, which is what actually turned the nightly red; that half is [#209](https://github.com/mephistopheles4/stacks/pull/209)'s, from another session, and this branch is stacked on it. See [the log](./log/2026-08-22-the-summary-that-grew-a-byte-per-mutant.md) |
...
| Resolved versions | TS **6.0.3, pinned exactly** since [ADR-0064](./adr/0064-typescript-6-until-7-1.md) — was 7.0.2 · Vitest 4 · Astro 7.1.6 · three 0.185.1 · sharp 0.35 · Stryker 9.6.1 (pinned exactly) · Prometheus 2.55.1 (the trend store's image) |
