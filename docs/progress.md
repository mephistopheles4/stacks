...
| **In flight, not yet green** | G40 (`action-pins`), G41 (`gate-register`) and G42 (`dependency-audit`) on `claude/mattpocock-skills-154-162-71abbe` — **three in one commit**, because `gate-register` shipped alone is red on every row and shipped against stubs is green over empty sections. ⚠️ **The branch is deliberately red on two assertions until it is rebased onto `metrics-freshness`'s row**, which lands ahead of it: G19's gapless check and G41's own floor. Both are the ordering dependency, not a defect. ⚠️ `gate-register`'s **first red was not planted** — G37 (`agents-import`) had landed out-of-band with no register entry, 37 entries against 38 rows. See [the log](./log/2026-08-20-the-register-gate-found-a-row-nobody-triaged.md) |
