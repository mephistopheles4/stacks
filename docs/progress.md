...
| **CRLF anywhere in a `.prom` file is a parse error** | ⚠️ *"invalid metric type \"gauge\r\""* — met by concatenating the records with PowerShell's `Set-Content`, not by the emitter, which writes `
`. Worth knowing before anything on Windows touches the record |
