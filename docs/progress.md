...
| | |
| --- | --- |
| **Last green gate** | G38 (`mutation-scope`) — the declared mutation scopes ↔ the tree they claim to score. **The first row that runs on two surfaces**: six structural clauses in `pnpm test`, and the zero-mutant residual as a refusal in `pnpm deploy:site`. ⚠️ The rollout's spec allocated it G37; `agents-import` took that number first, which is the third pre-allocated number in this effort to be wrong |
