...
| **Trend layer** | **live, and now twelve series.** The cognitive four — `cognitive-functions`, `cognitive-mass`, `cognitive-mass-over-15`, `cognitive-max` — landed beside the cyclomatic four on the same cadence ([#255](https://github.com/mephistopheles4/stacks/issues/255), [ADR-0073](./adr/0073-cognitive-complexity-is-published-beside-cyclomatic.md)). ⚠️ **Its own denominator**, and the predicted nine-node gap reproduced exactly on the real tree: 1133 cyclomatic functions against 1124 cognitive, all nine in `packages/site/src/shelf`. #230's two headline pairs reproduced too — `enrichBook` at CC41/cog75 and `inspectPublicBuild` at CC57/cog80. ⚠️ **The `fixtureHash` changed once at adoption**, by design: one hash over both counting rules, costing the 10 records that carried the old stamp and restarting both cyclomatic calibration windows from zero. Nothing armed, nothing refused. ⚠️ **No name joined `CAPPED_SERIES`** — that is [#258](https://github.com/mephistopheles4/stacks/issues/258), twenty records later. `.github/workflows/metrics.yml` writes one `.prom` per run to the orphan **`metrics`** branch — `git fetch origin metrics` to read it. Its first nightly found a scope 6.45 points down on a false comment in the PR that built it; see [the log](./log/2026-08-19-the-first-nightly-caught-its-own-author.md). `pnpm trend:sync` is the reading half and is **built** — it replays the branch into a local Prometheus, folds surface D in, and brings up the page you read at <http://localhost:3000/d/stacks-trend-layer>, provisioned from [`grafana/`](../grafana); needs Docker, see [`commands.md`](./commands.md) |
