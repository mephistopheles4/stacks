...
book cover actually is: hard-edged type over a large flat saturated field. *The
Five Dysfunctions of a Team* is white serif on `#c8102e`, which is the worst case
...
argument at the time was about *pixels* — 314 MB of decoded texture, and how
...
in *decoded* pixels and the dimensions are unchanged — the only cost is bytes on
...
fix and one of them written *for* the trap: a JPEG over the cap comes out 4:4:4,
