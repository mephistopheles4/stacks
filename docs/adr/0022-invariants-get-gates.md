...
Every rule in CLAUDE.md has a named test that can go red, scored in [`docs/gates.md`](../gates.md). A rule nothing can fail on is a comment, and a gate never *observed* failing is not yet a gate.
...
- **2026-07-31** — **The invariants get gates, and the gates get a scoreboard** — [`docs/gates.md`](../gates.md). A pre-publication review found six documented rules had quietly stopped being true with nothing going red, including a Decision Log entry below that is false in one of its two call paths. The rule this project already had — *"a gate never observed failing is not yet a gate"* — now applies to the invariants themselves, not just to phase gates.
