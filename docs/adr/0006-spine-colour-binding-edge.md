...
- **2026-07-31** — **Spine colour is sampled from the cover's left edge, not the whole cover.** On a real book the printed sheet wraps continuously around the spine, so the strip nearest the binding _is_ the spine — a cover that is mostly white with a colour band down one side has a coloured spine. Falls back to the whole cover when the edge is nothing but paper (padded cover images). Owner's call: "as close as possible to the real book spine".
