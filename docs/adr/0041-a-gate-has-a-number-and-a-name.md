# A gate has a number *and* a name, and the name is anchored to its spec

Every scoreboard row carries a **Name** column holding a kebab-case slug, and
citations spell both: *"See docs/gates.md, row G19 (constitution-scoreboard)."*
...
leaves a visible hole — *"Mark it, do not delete it… a deleted row takes with it
the fact that the rule was ever considered."* A name cannot encode absence. Drop
...
So: number *and* name, which is the convention this directory already uses —
...
constitution ↔ the scoreboard`. What was missing was a *stable, greppable* form
...
[ADR-0026](0026-constitution-is-gated-not-duplicated.md) is about: *"a rule
written down twice is a rule that will be true in one place and false in the
other."* Adding names without anchoring them would have created 29 new
...
- **G5** (`vault-is-truth`) and **G13** (`no-third-party-material`) both name
  `gates/repo-hygiene.test.ts`, so neither *uniquely* claims the stem and the
...
*parseable* slug; every citation names the row's **current** slug; and enough
...
Citations are found by *line* rather than by the phrase `row Gn`, because
