...
_powered by Google_ graphic is different and is vendored **unaltered**.
...
unmodified, and Google's says _"Do not change any of the Google marks in any
way."_ A monotone recolour is a modification, so these glyphs do not sit inside
...
_to_ the mark's owner is an ordinary pattern, and published guidelines are
...
map recorded — every grant found is permission to _use_ artwork and silent about
_redistributing_ it — does not arise for a glyph drawn here. So this is a
trademark-_form_ question in place of a trademark-_redistribution_ question, not
one stacked on the other.

**Owner's decision** — _"i dont see risk in using the logos. i want monotone
svgs"_ — taken with the above put to them. The fallback if either provider ever
...
glyph cannot say _which_ provider without a tooltip, and a tooltip never fires
...
[#106](https://github.com/mephistopheles4/stacks/issues/106) dropped _"The Apple
logo is a trademark of Apple Inc., registered in the U.S. and other
countries."_ from `/attribution` because the card had locked the _icon_, and it
attached the condition in as many words: _"If the vendored icon artwork turns out
to carry the Apple logo, that sentence returns; that is a check on the asset, not
a reopened decision."_ It does. The route carries both sentences.
...
because the clause says _graphic_ and _"do not change any of the Google marks"_
...
`loading="lazy"`** — deferring a graphic whose premise is _always displayed_ is a
