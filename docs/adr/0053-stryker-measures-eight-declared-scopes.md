...
[`gates.md`](../gates.md) parked mutation testing under *Not gated, deliberately*
with an explicit revisit condition — *"Revisit once the rows above are green"* —
...
revisit found.** It says *"133 tests in ~2s"*; measured, the suite is **636 tests
across 66 files in 5.52s**. Mutation cost scales with suite runtime, so the
premise that made Stryker *"genuinely cheap"* is weaker than the file recorded.

## `9.6.1` exactly — `^9.6.0` would be a correctness bug

`9.6.1` (2026-04-10) is the *first* release that works against Vitest 4. Its
changelog entry is *"vitest-runner: fix vitest runner mutant hitcount and
coverage for v4.1"*, closing an issue titled *"No proper coverage with Vitest 4 …
completely uncovered functions are mutated and affect the resulting score."*
...
the *evidence* for it is the changelog and the code, never the range.
...
this effort found of a range *asserting* a compatibility that does not exist.
...
Each of these contradicts what the published packages *say*, in the same
...
- **`tsconfigFile` names a file that is not in the project.**
  ...
  ⚠️ **Safe *here*, and the qualification is the transferable half.** The
...
- **`plugins` names the runner explicitly.** Stryker's default plugin glob loads
  ...
  test-runner process resolves none of them and reports *"no TestRunner plugins
  were loaded"*.
...
which part of the tree moved, and *which scope moved* is the entire signal this
...
**1,227 mutants at 71.7%**. The *directory rollup* of the same name is **3,301 at
...
and rejected: it derives the scope *structure* from the measurement, which is the
...
`NoCoverage` and both score 0. Excluding *"anything at 100% NoCoverage"* would
...
is empty.** *"It is only glue"* is the most available excuse for excluding
...
`scripts/lib/repo-root.ts` is read as text rather than imported. And *"no spec
imports it"* is a mechanism **the attacker controls**, unlike *"the child never
sees the active mutant"*, which is a property of the harness — recorded as the
...
measurement is why.** §5 filed it under *"no spec imports them"*. True of direct
...
*omission*** — not `100`, not `NaN`. Measured three ways:
...
reading a *score* for an empty scope can never fire, because there is no row to
...
of the tree**, and the score would have been confidently wrong. *A gate that
cannot tell it is being run against the wrong tree is not yet a gate either.*
...
*"A gate is the last thing in the chain, so nothing can notice it being wrong"*
...
import it, and it has no spec of its own.** Those 20 gates *are* its oracles, so
...
- **`vitest.stryker.config.ts` exists and removes exactly one spec**, and the
  ...
  Stryker's runner hardcodes. That removal is *why* `packages/cli/src/env.ts`
