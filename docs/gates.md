...
| **G17** | `deploy-branch` | a deploy publishes `main`, or says why not. ⚠️ **The arm asserting the guard lets `main` through executes only on the push run after a merge**: `gates.yml` runs on `pull_request`, `actions/checkout` builds `refs/pull/N/merge`, so `branch === 'main'` is never true on a pull request — **every pull request is green on it by construction, including one that breaks it**. ⚠️ **So this row fails the merge rather than the change**: in a merge train it reddens `main` between merges and blocks whoever is queued, for a defect in nobody's diff. **Two incidents, two mechanisms, one cause** — an assertion that could not hold on a CI checkout, which refused on the absent metrics mirror before reaching its sentinel ([#210](https://github.com/mephistopheles4/stacks/pull/210)); then `Test timed out in 5000ms` on `27248ab`, the series count having grown 8 → 20 underneath it ([#270](https://github.com/mephistopheles4/stacks/issues/270)). A third is the same cause, not bad luck. **The discriminator**: on a red `main` here, re-run the failed jobs unchanged before investigating, and after merging anything, check the push run and not only the pull request | `gates/deploy-branch.test.ts` — its last test reads the real checkout and carries a 30s timeout of its own, headroom against the whole file's worst observed CI run (8951ms). ⚠️ **Its speed rests on the absence of a ref**: G39 refuses right after step 0b because no `actions/checkout` has `refs/remotes/origin/metrics`, and a workflow that fetches it reopens the question | ✅ |
