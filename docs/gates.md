...
| **G35** | `enhanced-card` | the card a browser builds, not the model behind it | *"the card opened"* was the whole assertion, and it stays true through a card with no reading line, links with no accessible name, and an announcer that never changes | `scripts/smoke-render.ts` — `cardFailures`, against `docs/spec/enhanced-card.md` §11 | ✅ |
