...
| **G60** | `one-shadow-reader` | exactly one program reads the real-time shadow map on the default page, in a constant number of draws a frame at any library size — at most `BUDGET`, 4. On the Pixel 10 Pro XL the WebGL context is lost in a **count of draws whose program samples the map**: five programs died at frame 8, one survives 12 a frame and dies at 13, and a second one lowers that line ([#381](https://github.com/mephistopheles4/stacks/issues/381), [ADR-0088](adr/0088-one-program-samples-the-shadow-map-in-two-draws.md), [ADR-0090](adr/0090-real-time-shadows-are-the-default.md)). So a book program that starts reading the map again, or a bookcase split back into a draw per member, would ship green everywhere and lose the context on that phone for every visitor — no gate read what the renderer compiles. ⚠️ **It pins the configuration that survived, not survival**: the budget is one driver's edge divided by three, and another GPU could have a lower one with nothing here going red; see [its section](#g60--the-configuration-that-survived-not-survival) | `scripts/smoke-render.ts` — `checkShadowReaders`: the bare default page at the 50-book fixture and at a generated 300-book one, 17 shelves tall, and `?receivers=all` as the control that must come back red for the right reason, each in a browser context of its own with the counting hook in `scripts/lib/sampling-hook.ts` installed before any page script, over the judge in `scripts/lib/shadow-sampling.ts`. Eight clauses: the hook is live and the shadow pass drew at least one draw per book; the program set settled for 30 frames; exactly one sampling program in every steady frame; at most 4 sampling draws in every frame, onset included, an instanced call counted as its instances and a multi-draw call as its sub-draws; GL and the shader source agree about every program; the hook's draw count equals three's; the large page has its books and at least 8 shelves; the control is red on programs and on draws. Every clause is planted in `scripts/lib/shadow-sampling.test.ts`, the hook's behaviour in `scripts/lib/sampling-hook.test.ts`, and the string a page runs in `scripts/lib/sampling-hook-source.test.ts`. ⚠️ **The judge never reads the page, and the page never reads the judge**: a green needs both halves, and the control is what proves the two still meet | ✅ |
...
> September the failure turned out to be a count of draws whose program samples
> the map, one program survives in a handful of draws, and real-time shadows
...
> ⚠️ **2026-09-25: still true, and narrower.** G60 now gates the *trigger* the
> phone measured — how many programs read the shadow map, and in how many draws
> — on a desktop, because three builds the programs in JavaScript. Survival is
...
**Why the budget is 4.** One sampling program held 11 draws a frame in every
run and 12 in three of three; 13 died twice at frame 134. A run capped at 12 by
skipping draws died where an uncapped 12 lived, so the edge is not a margin to
spend. 4 is a third of it: the bookcase makes 2, and 2 are left for a member
that one day needs a mesh of its own. `BUDGET` carries the citations. **A red
is not answered by raising it**, which is G15's rule: find the program that
started sampling, then run `scripts/phone-check.ts` on a phone.
...
| **The shelf survives on real hardware** | **It needs a device, and CI has none.** Two separate phone failures shipped past every gate here — 314 MB of texture in August, and a driver that loses the context in a count of draws that sample the shadow map — because `smoke:render` renders on a desktop with gigabytes of headroom. G15 (`cover-budget`) now gates the first cause and G60 (`one-shadow-reader`) the second's measured trigger, on one driver, and neither is survival: another GPU could fail differently with both green. `scripts/phone-check.ts` runs the page on a USB-attached Android phone and gives the same verdict every time, and it is a tool run by hand rather than a `pnpm` script — adb and a phone are not something [`CONTRIBUTING.md`](../CONTRIBUTING.md) can ask of a contributor, the precedent `scripts/gh-post.ts` set. Run it before merging anything that touches shadows, materials, lights, the bookcase's geometry or three |
