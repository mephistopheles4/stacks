...
- **2026-09-25** — **`tsx` rewrites a function you serialise.** It compiles with esbuild's `keepNames`, so `fn.toString()` carries calls to a module-level `__name` helper, and a page that evaluates it throws `__name is not defined`. Vitest's transform need not do the same, so a spec of the serialised hook can pass while the gate's page throws. The shim is in the source string; the gate's first clause is what notices it going stale.
