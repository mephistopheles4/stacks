...
| **G46** | `lint` | a floating promise, an unhandled union case or an unsafe use of `any` reaching `main`. ESLint has run here since the complexity counter landed and **was not a linter**: one rule at a threshold nothing can satisfy, so its report is a population and never a verdict. Nothing read the source for a defect the compiler does not catch — `tsc --noEmit` passes on all 33 findings this reported on arrival, across 19 files, and 9 of them are in `scene.ts`, which sits outside every mutation scope and every complexity scope. ⚠️ **This row does *not* overlap G6 (`site-core-imports`).** The two rules that reproduce G6 exactly, on all four of its documented cases, are `no-restricted-imports` and `no-import-type-side-effects` — measured on [#233](https://github.com/mephistopheles4/stacks/issues/233), **in none of the five presets, and deliberately not enabled here**. Enabling them is [#245](https://github.com/mephistopheles4/stacks/issues/245)'s question, and until it answers, the import rule is held by G6 alone | `pnpm lint` — `eslint.lint.config.mjs`, the type-checked recommended set tuned to four repository idioms plus `switch-exhaustiveness-check`, over every `.ts` file, tests included. Run by the `style` job in `.github/workflows/gates.yml`, beside `audit` rather than inside the `suite` matrix, because a lint verdict cannot move with the Node version. ⚠️ **Nothing gates the config**: four tuned options are four places a rule can be weakened later and no gate here reads a lint config — [recorded, not solved](#not-gated-deliberately) | ✅ |
