...
- **2026-08-23** — **A measurement of a live branch is stale the moment it is written down, and this one was wrong in a commit message before the commit was pushed.** Counting the records on `origin/metrics` to price the `fixtureHash` change gave **41 records, 10 stamped**. That went into an ADR, a `stryker.floors.json` comment and a commit body. About an hour later it was **43 and 12** — two pull requests had merged and each wrote a merge record. Nothing was wrong with the count; the branch moved under it. ⚠️ **A concurrent session caught it, not this one**, and also caught the more useful error underneath: the figure quoted was the wrong *quantity*. `capCalibration` walks `streakOf`, which starts from `nightliesIn`, and **only 1 of those 12 is a nightly** — so the calibration window went 1 → 0, not 10 → 0, and the alarming-sounding number described a set nothing reads. **Two rules fall out.** A count taken from a live remote is *as-of* and must say so, or carry no number at all — the three rhetorical *"the measure this repository has 41 records of"* asides in this diff were changed to carry none, because the number was doing no work and only rotting. And **quote the quantity the mechanism actually consumes**: `countedIn`'s set and `capCalibration`'s window are different filters over the same records, and citing the first while claiming a consequence about the second is a true sentence about the wrong thing. Related in kind to the 2026-08-01 README row — *a hardcoded count, in the one file with no gate* — with the twist that here the count was re-derived correctly each time and the *world* was the moving part.
