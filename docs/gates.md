...
and G60**, written for defects that had not happened (G17 and G60 because the
change each shipped with made one reachable — for G60, the default page
...
> fallback a device takes after a lost context. G60 (`one-shadow-reader`)
...
> ⚠️ **2026-09-25: still true, and narrower.** G60 now gates the *configuration*
...
## G60 — the configuration that survived, not survival

G60 (`one-shadow-reader`) is the first row here about a GPU, and it cannot
