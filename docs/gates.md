...
| **G59** | `context-loss-fallback` | a lost WebGL context while the shelf samples the shadow map ends with the shelf drawn again **painted**, once, and remembered — never with the sampling scene resumed. The old path resumed it: three re-links the same programs on a restore, and on the Pixel 10 Pro XL two restores ~1.15 s after a loss died again after 3,088 and 3,474 more sampling draws ([#381](https://github.com/mephistopheles4/stacks/issues/381), [ADR-0091](adr/0091-a-lost-context-falls-back-to-painted-shadows.md)). Every clause is a line whose removal still draws a desktop shelf: a disposed shelf's render loop surviving beside the new one, a rebuild on the lost canvas, a record that never lands, one that lands from a painted page. ⚠️ **A staged loss is not a driver's**: `WEBGL_lose_context` never restores on its own, never exits the GPU process and never gets the page blocked, so this proves the page's side and nothing about Chrome's — on the Pixel a real loss was refused a new context in 4 of 4 runs, recorded in the log, not here | `scripts/smoke-render.ts` — `checkContextLossFallback`, four cases each in a browser context of its own: a restore rebuilds painted on the same canvas with **one** render loop, counted per animation frame, then a reload starts `remembered` and `?shadows=1` still wins; no restore rebuilds on a new element, drawn and clickable; storage refusing every write still falls back and says *not remembered* with no page error; a painted page's loss writes nothing, rebuilds nothing and resumes in place. The page it loses is read off `DEFAULT_SETTINGS`, so the cases keep sampling the map when the default flips. The decisions are unit-specced in `packages/site/src/shelf/context-recovery.test.ts` and `packages/site/src/shelf/shadow-fallback.test.ts`. ⚠️ **The two `disposed` guards cover each other**, so the loop clause goes red only when both are gone — measured, and deliberate | ✅ |
