...
**Why the settle.** While a woodwork sheet or a cover decodes, the bookcase's
no-map program and its mapped one both draw for a frame or two, and relink. So
"exactly one" is judged over the frames after the last link, from the page's
ready frame on, and needs 30 of them; the budget applies to every frame,
onset included.
