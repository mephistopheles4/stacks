...
function documentedKeys(): string[] {
  const section = markdownSection(
    readRepoFile(AGENTS_DOC),
    "Frontmatter contract",
    AGENTS_DOC,
  );

  const enumeration = section
    .split("\n")
    .find((line) => line.startsWith("Required:"));
  if (enumeration === undefined) {
    throw new Error(
      'no line starting "Required:" in the Frontmatter contract section',
    );
  }
...
describe("G8 — frontmatter contract", () => {
  ...
  it("reads every contract key into the field it names", () => {
    for (const key of CONTRACT_KEYS) {
      ...
      if (key !== "type" && key !== "title")
        lines.push(`${key}: ${entry.sample}`);

      const parsed = parseNote(noteWith(lines), `${key}.md`);
      expect(
        parsed.kind,
        `\`${key}: ${entry.sample}\` should parse as a book`,
      ).toBe("book");
  ...
  it("leaves an optional field undefined when its key is absent", () => {
    const bare = parseNote(
      noteWith(["type: book", "title: A Sample Title"]),
      "bare.md",
    );
    ...
    for (const key of CONTRACT_KEYS) {
      const entry = FRONTMATTER_CONTRACT[key];
      if (entry.field === null || entry.required || ALWAYS_PRESENT.has(key))
        continue;
