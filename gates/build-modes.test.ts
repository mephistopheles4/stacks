...
const INTENDED_DIFFERENCES = {
  ...
  cover:
    "rewritten to covers/<filename> so a public build is always same-origin",
} as const;
...
async function bothModes(): Promise<{
  ...
  try {
    const local = buildLibrary(books, { isPublic: false })
      .books as unknown as Record<string, unknown>[];
    const result = await publish(books, vault, assets, { isPublic: true });
    return {
      local,
      publicBuild: result.library.books as unknown as Record<string, unknown>[],
    };
  } finally {
...
describe("G11 — build modes", () => {
  it("ships every shipping book, in the same order, dropping the rest", async () => {
    ...
    expect(publicBuild.map((book) => book["id"])).toEqual(
      shipping.map((book) => book["id"]),
    );
  });

  it("never ships a book you do not own, or one marked private", async () => {
    const { publicBuild } = await bothModes();
    expect(publicBuild.every((book) => book["status"] !== "wishlist")).toBe(
      true,
    );
  ...
  it("differs only on the keys the difference is documented for", async () => {
    ...
    for (const [index, localBook] of shipping.entries()) {
      ...
      const keys = new Set([
        ...Object.keys(localBook),
        ...Object.keys(publicBook),
      ]);
  ...
  it("strips sourcePath from a public build and keeps it locally", async () => {
    ...
    expect(publicBuild.every((book) => book["sourcePath"] === undefined)).toBe(
      true,
    );
  });
  ...
  it("stamps coverAspect on a public build, for books whose cover shipped", async () => {
    ...
    expect(
      withCover.length,
      "fixture vault should have books with covers",
    ).toBeGreaterThan(0);
    expect(
      withCover.some((book) => typeof book["coverAspect"] === "number"),
    ).toBe(true);
  });
});
