...
const ALLOWED = [
  ...
  // it rewrites is touched. It does not know what a book is. ⚠️ **The rewrite
  // itself is `restampConfigHash` in `lib/floors.ts` above, which stays pure**:
  // that entry's *"it writes nothing"* is still true, and this is the file the
  // write moved into rather than a second one growing beside it.
  'scripts/stamp-floors.ts',
