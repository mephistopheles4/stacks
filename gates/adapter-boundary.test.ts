 ...
 */
const FS_IMPORT =
  /(?:from|import|require)\s*\(?\s*['"]((?:node:)?fs(?:\/promises)?)['"]/;
...
describe("G1 — adapter boundary", () => {
  ...
  it("lets no unlisted file reach the filesystem directly", () => {
    ...
    const offenders = governedFiles().filter(
      (path) => !allowed.has(path) && importsFs(path),
    );
  ...
  it("keeps every allowlist entry on a file that still imports fs", () => {
    ...
    const present = new Set(
      ALLOWED.filter((path) => existsSync(join(REPO_ROOT, path))),
    );
  ...
  it("still finds fs in the adapter itself, so the detector is not broken", () => {
    ...
    expect(importsFs("packages/core/src/adapters/obsidian-adapter.ts")).toBe(
      true,
    );
  });
});
