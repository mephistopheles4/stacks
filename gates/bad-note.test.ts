...
import { parseNote, type ParsedNote } from '../packages/core/src/frontmatter.ts';
...
const BINARY = Buffer.from([
  0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a, 0x00, 0x00, 0x00, 0x0d, 0xff, 0xfe, 0x00, 0x01,
]).toString('utf8');
...
describe('G3 — never crash on a bad note', () => {
  for (const { name, source, expected, why } of CORPUS) {
    it(`returns ${expected} for ${name}`, () => {
      ...
      if (parsed.kind === 'invalid') {
        ...
        expect(parsed.reason.length, 'an invalid note must say why').toBeGreaterThan(0);
      }
    });
  }
  ...
  it('covers all three outcomes, so no single blanket answer satisfies the corpus', () => {
    ...
    expectFound([...kinds], 'distinct ParsedNote kinds exercised by the corpus', 3);
