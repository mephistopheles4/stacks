...
import {
  ...
  SPINE_LANDED,
  judgeRecord,
} from "../scripts/lib/metrics-read.ts";
 ...
 */
function repoWith(
  stored: readonly Planted[] | undefined,
  branch?: readonly Planted[],
): string {
  ...
  const commit = (records: readonly Planted[]): string => {
    ...
    for (const record of records)
      writeFileSync(
        join(dir, RECORD_DIR, record.name),
        record.document,
        "utf8",
      );
    git("add", "-A");
...
describe("G39 — the harness reaches the check at all", () => {
  it("gets past a fresh record, and prints it", () => {
    ...
    expect(
      output,
      "a fresh record must not be what stops the deploy",
    ).toContain(PAST_THE_CHECK);
...
describe("G39 — one fetch, two messages", () => {
  it("sends you to the sync when the branch has rows the store lacks", () => {
    const repo = repoWith(
      [nightly(4 * DAY)],
      [nightly(4 * DAY), nightly(600, "cccccccc")],
    );
    ...
    expect(
      refAt(repo, "refs/remotes/origin/metrics-probe"),
      "the probe ref is written",
    ).not.toBe("");
  });
...
describe("G39 — which flags clear it", () => {
  ...
  it("reports instead of refusing under --check-only, which uploads nothing", () => {
    ...
    expect(output).toContain("the trend record is stale");
    expect(output, "--check-only must reach the check it reports on").toContain(
      PAST_THE_CHECK,
    );
  });
});
