...
function headersFile(
  ...
  return [
    ...
    "",
    "/covers/*",
    options.coversCacheControl === false
...
function cspMeta(
  ...
  const keep = ([name]: readonly [string, string]): boolean =>
    name !== options.omit;
  const render = (
  ...
  const generated = [
    ...
    [
      "style-src",
      "'self' 'sha256-eZThLoRNDbx6YobQlsGZA8+qespsZfD/YQEGSwT5BvU='",
    ],
  ] as const;
...
describe("G20 — every rule goes red", () => {
  ...
  it("foreign-cover: a cover served by somebody else", async () => {
    await expectOnly("foreign-cover", async () => {
      ...
      await writeLibrary([
        {
          title: "A Book",
          cover: "https://elsewhere.example/a.jpg",
          status: "read",
        },
      ]);
      await rm(join(dist, "covers"), { recursive: true, force: true });
  ...
  it("orphan-cover: a cover no shipped book points at", async () => {
    await expectOnly("orphan-cover", async () => {
      ...
      await writeFile(
        join(dist, "covers", "a-real-book-you-actually-read.jpg"),
        "pretend jpeg",
      );
    });
  });
  ...
  it("csp: connect-src widened past 'self'", async () => {
    ...
    await expectOnly("csp", async () => {
      await writeIndex(
        indexHtml({
          csp: cspMeta({ connectSrc: "'self' https://telemetry.example" }),
        }),
      );
    });
  });
    ...
    await expectOnly("csp", async () => {
      await writeIndex(
        indexHtml({
          csp: cspMeta({
            scriptSrc:
              "'self' https://static.cloudflareinsights.com https://cdn.example",
          }),
        }),
      );
    });
  ...
  it("csp: a directive nobody named, widening the policy past `default-src`", async () => {
    ...
    await expectOnly("csp", async () => {
      await writeIndex(
        indexHtml({
          csp: cspMeta({ extra: "script-src-elem https://cdn.example" }),
        }),
      );
    });
  });
  ...
  it("headers: framing is no longer denied by the header policy", async () => {
    ...
    await expectOnly("headers", async () => {
      await writeFile(
        join(dist, "_headers"),
        headersFile({ frameAncestors: false }),
      );
    });
  });
  ...
  it("headers: framing is no longer denied to the older mechanism", async () => {
    ...
    await expectOnly("headers", async () => {
      await writeFile(
        join(dist, "_headers"),
        headersFile({ frameOptions: false }),
      );
    });
  });
  ...
  it("headers: covers that do not revalidate, with a neighbour that does", async () => {
    await expectOnly("headers", async () => {
      ...
      await writeFile(
        join(dist, "_headers"),
        headersFile({ coversCacheControl: false }),
      );
    });
  });
