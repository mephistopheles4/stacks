...
describe('coverFileName', () => {
  ...
  it('takes the last segment of a posix path', () => {
    expect(coverFileName('covers/the-tidal-engine.png')).toBe('the-tidal-engine.png');
  });

  it('takes the last segment of a windows path', () => {
    // The case the old hand-rolled `split('/')` returned whole.
    expect(coverFileName('covers\\the-tidal-engine.png')).toBe('the-tidal-engine.png');
  });

  it('strips a traversal written with either separator', () => {
    expect(coverFileName('../../../secrets.png')).toBe('secrets.png');
    expect(coverFileName('..\\..\\..\\secrets.png')).toBe('secrets.png');
    expect(coverFileName('../mixed\\separators/secrets.png')).toBe('secrets.png');
  });

  it('refuses a value that is only a directory reference', () => {
    for (const value of ['', '.', '..', '   ', 'covers/', 'covers\\', 'covers/..']) {
      expect(coverFileName(value), `\`cover: ${value}\` has no usable filename`).toBe('');
    }
  });
});
