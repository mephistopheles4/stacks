...
describe("G18 — download refuses what it should not decode", () => {
  ...
  it("refuses an error page served with HTTP 200", async () => {
    const html = Buffer.from(
      `<!doctype html><title>Not found</title>${"x".repeat(2000)}`,
    );
    stubFetch(
      respondWith(html, { "content-type": "text/html; charset=utf-8" }),
    );
  ...
  it("refuses non-image bytes even when the header claims an image", async () => {
    ...
    const html = Buffer.from(
      `<!doctype html><title>Nope</title>${"x".repeat(2000)}`,
    );
  ...
  it("refuses a declared length over the cap without reading the body", async () => {
    ...
    expect(
      bodyWasRead,
      "body should not be transferred once the header disqualifies it",
    ).toBe(false);
  });
  ...
  it("refuses a non-2xx response", async () => {
    stubFetch(
      respondWith(await realCover(), { "content-type": "image/jpeg" }, 404),
    );
