...
async function coverShaped(): Promise<Buffer> {
  return await sharp({ create: { width: 400, height: 600, channels: 3, background: '#2f6d7a' } })
    .jpeg()
    .toBuffer();
}
...
async function spreadShaped(): Promise<Buffer> {
  return await sharp({ create: { width: 1400, height: 600, channels: 3, background: '#7a4b2f' } })
    .jpeg()
    .toBuffer();
}
...
describe('cacheCover — which candidate wins', () => {
  let dir: string;
  ...
  it('passes over a jacket spread for the next candidate', async () => {
    ...
    const cached = await cacheCover([SPREAD, COVER], 'The Tidal Engine', vault);
  ...
  it('records the provider of the URL that won, not the one that was asked first', async () => {
...
describe('cacheCover — nothing to try', () => {
  let dir: string;
  let vault: ObsidianAdapter;
  ...
  it('tries the candidates that are present and ignores the gaps', async () => {
    ...
    const cached = await cacheCover([undefined, COVER], 'The Tidal Engine', vault);
  ...
  it('returns undefined when every download fails', async () => {
    ...
    expect(await cacheCover([DEAD], 'The Tidal Engine', vault)).toBeUndefined();
  });
});
