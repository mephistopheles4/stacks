...
describe('isBlank', () => {
  ...
  it('keeps a pale cover that still has something on it', async () => {
    ...
    const pale = await sharp({
      ...
      .composite([
        {
          input: await sharp({
            create: { width: 300, height: 200, channels: 3, background: '#404040' },
          })
            .png()
            .toBuffer(),
          top: 200,
  ...
  it('keeps every real fixture cover', async () => {
    for (const file of ['the-tidal-engine.png', 'the-salt-road-ledger.png', 'white-bordered.png']) {
      expect(await isBlank(await sharp(cover(file)).toBuffer()), file).toBe(false);
    }
  });
