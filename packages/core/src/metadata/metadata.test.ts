...
describe("O'Reilly, for the books the other three do not have", () => {
  ...
  it("records those bytes as oreilly, so a takedown can name them", async () => {
    expect(
      coverSourceFor(
        "https://learning.oreilly.com/covers/urn:orm:book:123/1200w/",
      ),
    ).toBe("oreilly");
...
describe("API miss", () => {
  ...
  it("returns no results for a search that matches nothing", async () => {
    expect(
      await searchByTitle("zzzqqqxx no such book anywhere", openLibraryMiss),
    ).toEqual([]);
  });
  ...
  it("degrades to not-found when the Google Books fallback is out of quota", async () => {
    ...
    expect(
      await lookupByIsbn("9790000000001", openLibraryMiss),
    ).toBeUndefined();
    expect(
      await lookup("zzzqqqxx no such book anywhere", openLibraryMiss),
    ).toEqual([]);
  });
            ...
            items: [
              {
                volumeInfo: {
                  ...
                  industryIdentifiers: [
                    { type: "ISBN_13", identifier: "9780691249148" },
                  ],
                  imageLinks: {
                    thumbnail: "http://books.google.com/real-cover",
                  },
                },
              },
            ],
