...
const appleHit = fixtureHttpGet({ 'itunes.apple.com': 'apple-search-hit.json' });
const appleNearMiss = fixtureHttpGet({ 'itunes.apple.com': 'apple-search-near-miss.json' });
...
describe('Open Library keeps what it always sent', () => {
  it('takes publisher, publication date, subjects and the OLID from the ISBN response', async () => {
    const result = await openLibrary.lookupByIsbn(CAPTURED_ISBN, openLibraryHit);
...
describe('Apple stops throwing its match away', () => {
  it('returns the matched record, not one artwork URL', async () => {
    const result = await appleBooks.findRecord('Atomic Habits', 'James Clear', appleHit);
