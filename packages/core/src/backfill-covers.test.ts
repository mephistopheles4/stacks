...
function measuring(
  shapes: Record<string, { width: number; height: number }>,
): MeasureCover {
  return (path) => {
...
describe("backfillCoverSources", () => {
  it("records a source inferred from the cover it can measure", async () => {
    ...
    const written = await readFile(
      join(vaultPath, "Library", "Apple Sized.md"),
      "utf8",
    );
  ...
  it("writes unknown when the shape is not diagnostic", async () => {
    ...
    expect(
      await readFile(join(vaultPath, "Library", "Ambiguous.md"), "utf8"),
    ).toContain("cover_source: unknown");
  });
  ...
  it("never overwrites a source that was recorded at fetch time", async () => {
    ...
    expect(
      await readFile(join(vaultPath, "Library", "Observed.md"), "utf8"),
    ).toContain("cover_source: open-library");
  });
  ...
  it("leaves the note untouched in a dry run", async () => {
    ...
    expect(await readFile(join(vaultPath, "Library", "Dry.md"), "utf8")).toBe(
      before,
    );
  });
  ...
  it("reports a cover it cannot measure rather than guessing", async () => {
    ...
    expect(
      await readFile(join(vaultPath, "Library", "Missing.md"), "utf8"),
    ).not.toContain("cover_source");
  });
  ...
  it("preserves the note body and the rest of the frontmatter byte for byte", async () => {
    ...
    const after = await readFile(
      join(vaultPath, "Library", "Careful.md"),
      "utf8",
    );
