...
describe("normaliseTitleAuthor", () => {
  it("collapses the things that differ between sources for one book", () => {
    expect(normaliseTitleAuthor("The Salt Road Ledger")).toBe(
      "salt road ledger",
    );
...
describe("isProbablySameBook", () => {
  ...
  it("does NOT match a summary or study guide of the same book", () => {
    ...
    expect(
      isProbablySameBook(
        "Workbook for Atomic Habits James Clear",
  ...
  it("does NOT collapse two different books by the same author", () => {
    ...
    expect(
      isProbablySameBook(
        "The Tidal Engine Marisol Vane",
        "The Quiet Protocol Marisol Vane",
      ),
    ).toBe(false);
    expect(
      isProbablySameBook("Compilers for the Impatient Roy", "Lantern Work Roy"),
    ).toBe(false);
  });
...
describe("titleMatchScore", () => {
  ...
  it("scores an unrelated title at zero", () => {
    expect(
      titleMatchScore("salt road ledger", "Compilers for the Impatient"),
    ).toBe(0);
  });
});
...
describe("rankingScore", () => {
  ...
  it("still favours the named author when the query names one", () => {
    const right = rankingScore(
      "Staff Engineer Will Larson",
      "Staff Engineer",
      "Will Larson",
    );
    const wrong = rankingScore(
      "Staff Engineer Will Larson",
      "Staff Engineer",
      "Someone Else",
    );
  ...
  it("scores an unrelated title at zero", () => {
    expect(
      rankingScore("salt road ledger", "Compilers for the Impatient"),
    ).toBe(0);
  });
});
