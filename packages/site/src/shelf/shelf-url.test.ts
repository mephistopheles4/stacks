...
const read = (query: string) =>
  resolveSettings(readSettings(new URLSearchParams(query)));
...
describe("the ten probes", () => {
  ...
  it("refuses a shadow type it does not know", () => {
    ...
    expect(read("shadowtype=fancy").shadows.type).toBe(
      DEFAULT_SETTINGS.shadows.type,
    );
  });

  it("refuses a nonsense pixel ratio or map size", () => {
    expect(read("dpr=0").renderer.maxPixelRatio).toBe(
      DEFAULT_SETTINGS.renderer.maxPixelRatio,
    );
    expect(read("dpr=x").renderer.maxPixelRatio).toBe(
      DEFAULT_SETTINGS.renderer.maxPixelRatio,
    );
    expect(read("shadowmap=1.5").shadows.mapSize).toBe(
      DEFAULT_SETTINGS.shadows.mapSize,
    );
    expect(read("shadowmap=-1").shadows.mapSize).toBe(
      DEFAULT_SETTINGS.shadows.mapSize,
    );
  });
});
...
describe("?tune", () => {
  it("carries a light change through", () => {
    ...
    expect(read(query).lighting.key.colour).toBe(
      DEFAULT_SETTINGS.lighting.key.colour,
    );
  ...
  it("carries tone mapping and exposure", () => {
    ...
    expect(read(query).renderer).toMatchObject({
      toneMapping: "aces",
      exposure: 1.4,
    });
  });
  ...
  it("refuses a tone mapping name it does not know", () => {
    ...
    expect(read(query).renderer.toneMapping).toBe(
      DEFAULT_SETTINGS.renderer.toneMapping,
    );
  });
  ...
  it("refuses a non-finite exposure", () => {
    ...
    expect(read(query).renderer.exposure).toBe(
      DEFAULT_SETTINGS.renderer.exposure,
    );
  });
  ...
  it("carries a nested profile without flattening it or losing its sibling", () => {
    ...
    expect(settings.materials.pageStriation).toBe(
      DEFAULT_SETTINGS.materials.pageStriation,
    );
  });
  ...
  it("does not let a tune blob override a probe typed by hand", () => {
    ...
    const tune = encodeURIComponent(
      JSON.stringify({ lighting: { key: { intensity: 4.5 } } }),
    );
