...
/**
 * How many world units one tile of the map covers.
 *
 * Poly Haven publishes `sapele_veneer` as 500 mm square. This scene's unit is
 * about 0.30 m — `MAX_HEIGHT` is 0.95 for a book that would stand about 290 mm
 * — so 0.5 m is about 1.6 units, and the veneer is laid at its true size rather
 * than at whatever looked nice. A 3.58-unit plank therefore shows the tile
 * about 2.2 times along its length, and that repetition is a cost of the
 * one-board answer #281 chose, not a defect here.
 */
const UNITS_PER_TILE = 1.6;
...
export function applyWoodArm(
  ...
  if (config.arm === 'flat') {
    material.color.setHex(SAPELE_MEAN);
    done();
    return;
  }
  ...
  if (wantsPigment) {
    material.map = load(`sapele-diff-${size}.jpg`, true);
  ...
  if (wantsRelief) {
    material.normalMap = load('sapele-nor-512.jpg', false);
    material.normalScale = new THREE.Vector2(config.normalScale, config.normalScale);
  }
  if (wantsRough) {
    material.roughnessMap = load('sapele-rough-512.jpg', false);
  }
