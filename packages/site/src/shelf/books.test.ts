...
describe("toRows", () => {
  ...
  it("never packs a row past the shelf it is packing into", () => {
    const rows = rowsOf(
      Array.from({ length: WRAPS }, (_, index) =>
        book(`book-${String(index)}`, { pages: 800 }),
      ),
    );
  ...
  it("opens a gap where the year changes, and counts it against the capacity", () => {
    ...
    const [row] = rows;
    expect(row?.books.map((entry) => entry.gapBefore)).toEqual([
      undefined,
      0.09,
      undefined,
    ]);
  ...
  it("leaves wishlist books off the shelf — you do not own them", () => {
    ...
    expect(
      rows.flatMap((row) => row.books).map((entry) => entry.book.id),
    ).toEqual(["owned"]);
  });
...
describe("a book with no page count", () => {
  ...
  it("stays unremarkable, never claiming to be a doorstop or a pamphlet", () => {
    ...
    for (const thickness of thicknesses()) {
      expect(Math.abs(thickness - middle)).toBeLessThanOrEqual(
        (0.16 - 0.055) * 0.15 + 1e-12,
      );
    }
  });
  ...
  it("leaves a book that has a page count alone", () => {
    ...
    const measured = rowsOf([
      book("thin", { pages: 120 }),
      book("fat", { pages: 800 }),
    ]).flatMap((row) => row.books);

    expect(
      measured.find((entry) => entry.book.id === "thin")?.thickness,
    ).toBeCloseTo(0.055, 12);
    expect(
      measured.find((entry) => entry.book.id === "fat")?.thickness,
    ).toBeCloseTo(0.16, 12);
  });
});
...
describe("binding", () => {
  ...
  it("takes what the note declares, whatever the mixture says", () => {
    ...
    const declared = [
      book("a", { binding: "hardback" }),
      book("b", { binding: "paperback" }),
    ];
  ...
  it("hashes a binding for every book that declares none", () => {
    ...
    const paperbacks = bindingsAt(0.6).filter(
  ...
  it("moves the mixture with the ratio, in the right direction", () => {
    ...
    expect(bindingsAt(1).every((binding) => binding === "paperback")).toBe(
      true,
    );
  ...
  it("draws binding and height off independent hashes", () => {
    ...
    const tallestPaperback = Math.max(
      ...paperbacks.map((entry) => entry.height),
    );
    const shortestHardback = Math.min(
      ...hardbacks.map((entry) => entry.height),
    );
  ...
  it("biases the height band without widening it", () => {
    ...
    const paperbacks = mean(
      entries
        .filter((entry) => entry.binding === "paperback")
        .map((entry) => entry.height),
    );
    const hardbacks = mean(
      entries
        .filter((entry) => entry.binding === "hardback")
        .map((entry) => entry.height),
    );
