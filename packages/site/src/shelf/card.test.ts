...
describe('the reading line', () => {
  it('prefers finished over started', () => {
    expect(cardModel(book({ started: '2026-01-02', finished: '2026-03-04' })).reading).toBe(
      'read · finished 2026-03-04',
    );
  });

  it('falls back to started', () => {
    expect(cardModel(book({ status: 'reading', started: '2026-01-02' })).reading).toBe(
      'reading · started 2026-01-02',
    );
  });
...
describe('the object line', () => {
  it('takes catalogue order — publisher, published, pages, binding, isbn', () => {
    ...
    expect(model.object).toBe('Chelsea Green · 2008 · 240 pages · paperback · 9781603580557');
  });

  it('shows the ISBN as a visible string in its own right', () => {
    expect(cardModel(book({ isbn: '9781603580557' })).object).toBe('9781603580557');
  });
...
describe('subjects', () => {
  it('splits on the separator core joined with, not on a comma', () => {
    ...
    expect(cardModel(book({ subjects: 'Health, Mind & Body; Psychology' })).subjects).toBe(
      'Health, Mind & Body · Psychology',
    );
  });

  it('keeps owner tags and provider subjects apart', () => {
    const model = cardModel(book({ tags: ['fantasy'], subjects: 'Fiction / Fantasy' }));
...
describe('the announcement', () => {
  it('carries title and author, and nothing else', () => {
    ...
    expect(announcement(book({ author: 'An Author' }))).toBe('A Book by An Author');
